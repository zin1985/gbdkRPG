...
const char *battle_format_damage_message(const char *prefix, UINT16 value) BANKED {
    ...
    /*
     * rpg098:
     * Keep the rpg095 two-line style:
     *   line1: action text
     *   line2: n ダメージ!
     */
    si = 0u;
    ...
    if (prefix != 0) {
        while (prefix[si] != '\0' && glyphs < 18u && di < 70u) {
            c = (UINT8)prefix[si];
            if (c == (UINT8)'\n' || c == (UINT8)'\r') break;
            ...
            if ((UINT8)(di + len) >= 90u) break;
            for (j = 0u; j < len; j++) {
                if (prefix[(UINT8)(si + j)] == '\0') break;
                battle_message_buffer[di++] = prefix[(UINT8)(si + j)];
            }
            si = (UINT8)(si + len);
            glyphs++;
        }
    ...
    if (di < 90u) battle_message_buffer[di++] = ' ';

    /* UTF-8 bytes for "ダメージ" */
    battle_message_buffer[di++] = (char)0xE3u;
    battle_message_buffer[di++] = (char)0x83u;
    battle_message_buffer[di++] = (char)0x80u;
    battle_message_buffer[di++] = (char)0xE3u;
    battle_message_buffer[di++] = (char)0x83u;
    battle_message_buffer[di++] = (char)0xA1u;
    battle_message_buffer[di++] = (char)0xE3u;
    battle_message_buffer[di++] = (char)0x83u;
    battle_message_buffer[di++] = (char)0xBCu;
    battle_message_buffer[di++] = (char)0xE3u;
    battle_message_buffer[di++] = (char)0x82u;
    battle_message_buffer[di++] = (char)0xB8u;
    battle_message_buffer[di++] = '!';
