...
- 通常ターン中のDISPLAY_OFF再描画を停止。
- カーソル移動をカーソルだけの差分更新に変更。
- コマンド欄の下段使用回数表示を削除。
- 敵ターン終了後の次コマンド画面を再正規化。


## rpg080_icons
- 戦闘パーティ表示のアイコンを、ユーザー添付の まほう / ゆうしゃ / そうりょ 画像へ差し替え
- 16x16の4スプライト構成で表示
- 戦闘中のみ NPC タイル領域へロードし、戦闘後は init_map_sprites() で通常状態へ復帰する前提
