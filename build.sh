#!/bin/sh
set -eu
# rpg194 clean before build: avoid stale .rel/.asm bank overlap artifacts
rm -f *.rel *.asm *.lst *.sym *.map *.noi *.gb 2>/dev/null || true


export PATH="/opt/gbdk/bin:$PATH"

OUT="rpg195_sprite_cleanup_battle_item_pages_fullwidth.gb"
MAP="${OUT%.gb}.map"
...
rm -f *.asm *.lst *.rel *.sym *.noi *.map *.ihx *.gb *.o *.lk *.adb
